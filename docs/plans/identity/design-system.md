...
- **예외 (의도적 유지)**: ① 말씀카드 **이미지 생성 경로**(`BibleCardBase` 등 — 색이 생성물에 박힌다)
  ② 카카오 브랜드 노랑(`#FFE812`·`#FEE500`) ③ 달력 일러스트 톤(`#DEE0F1`·`#BBBED4`)
...
## 신규 UI 품질 기준 (2026-08-03, 사용자 지시)

**신규로 만드는 UI 는 "운영에 내놓을 수 있는가"를 기준으로 자기검수한다.** 액션시트 1차본이
기준 미달로 반려된 사례에서 규칙화: 제목이 벽에 붙고, 파스텔 그라디언트에 흰 아이콘이 묻히고,
행과 시트의 대비가 없었다.

- 만들면 **스크린샷을 찍어 직접 본다** — DOM 검증만으로 통과시키지 않는다
- 신규 컴포넌트는 `frontend-design` 스킬 기준으로 설계: 의도적 타이포·명확한 대비·한 곳의 시그니처
- 카피도 디자인이다 — 시스템 용어가 아니라 사용자의 언어로 (예: 액션시트 제목 "오늘 무엇을 남길까요?")
- **"그라디언트 사각 타일 + 아이콘" 나열 금지** — 전형적인 AI 생성 티 (2차 반려 사유).
  행의 구분은 텍스트 위계로; 아이콘은 의미를 운반할 때만
- **CTA 버튼 라벨 앞 장식 심볼 금지** — 반짝이·연필 등 (기도카드 만들기 버튼 반려). CTA 는 라벨이 전부다
- primitive 의 기본값(패딩 등)을 확인하고 쓴다 — 이 레포의 shadcn 은 표준에서 변형돼 있다
