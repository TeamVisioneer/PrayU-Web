...
결정: **원고는 레포에 커밋(PR 리뷰) → 게시는 어드민**. 이미지는 Storage 업로드(현행) 유지하되
URL이 환경 종속이라 **레포 원고에는 이미지 URL을 넣지 않는다**.

- [ ] 어드민 보조 기능 — 이미지 **URL 직접 입력** + **원고 붙여넣기**(`lib/noticeDraft.ts` 파서)
- [ ] 첫 공지 원고 작성 (`docs/notices/`) — 이번 릴리스 사용자 대면 변경 5건 중 2~3건으로 추림
- [ ] 스크린샷 캡처 — 로그인 필요 화면은 **사람이 직접**(AI는 카카오 OAuth 통과 불가)
