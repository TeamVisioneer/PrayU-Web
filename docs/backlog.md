...
- [x] ~~**안드로이드 카카오 원탭 복원 (B′)**~~ — [#524](https://github.com/TeamVisioneer/PrayU-Web/pull/524) [#525](https://github.com/TeamVisioneer/PrayU-Web/pull/525) · Api [#64](https://github.com/TeamVisioneer/PrayU-Api/pull/64) · **prod web `v0.16.2` + Api `v1.0.1` (2026-09-07, 안드로이드 실기기 검증)**. 계획서: [archive/kakao-android-onetap.md](archive/kakao-android-onetap.md). 후보 비교(기록용):
...
- [x] ~~**카카오톡 문의하기 — 안드로이드 앱에서 무반응 수리**~~ — #528 #531 · **prod `v0.16.3`→`v0.16.4` (2026-09-07, iOS·Android 실기기 확인)**. 경위: `window.open` 을 앱이 버린다 — flutter_inappwebview 는 `onCreateWindow` 미구현 시 iOS 만 같은 창에 로드해 주고(그래서 iOS 는 채널 페이지가 뜸) Android 는 무반응(플러그인 소스 실측). 수리: `lib/kakaoChannel.ts` — 앱에서는 SDK Channel.chat 과 같은 스킴/intent 로 **카카오톡 채팅방을 직접** 연다(PrayU 화면 유지, 돌아와도 채널 페이지 안 남음). 브라우저는 새 탭 그대로. 진입점 5곳(그룹 메뉴·그룹 없음·그룹 한도·QT 신고·약관) 통일. v0.16.3 출고 → iOS 에서 "비공개 또는 삭제된 채널" — 카카오 페이지가 항상 붙이는 `?fromtype=web` 누락이 원인(staging 스킴 변형 실기기 대조: iOS 는 플래그 없는 변형만 실패, Android 무관) → **v0.16.4** 수정. 근본(`onCreateWindow`)은 App R1/W1
- [ ] **후속: 로그인 후 앱 자동 복귀** — 지금은 iOS·Android 모두 완료 카드("PrayU 앱으로 돌아가면 로그인이 완료돼요") → 수동 복귀. 자동 복귀 = App Links/Universal Links 검증(web `assetlinks.json` 서명 지문 비어 있음 · AASA) + 앱 딥링크 수신 수리 → App [plans/deeplink-push-review-plan.md](../../PrayU-App/docs/plans/deeplink-push-review-plan.md) 의 결과물. 수용 기준에 "카카오 로그인 후 앱 자동 복귀" 추가. 크롬이 카카오톡발 리다이렉트에서 앱 링크로 점프하는지는 실기기 확인 필요(안 되면 앱 안에서만 `prayu://` redirect_to)
- [x] ~~**그룹 초대 → 로그인 → 초대 페이지 복귀 실패 (2026-09-07)**~~ — 원인은 코드가 아니라 Supabase **Redirect URLs glob**: `https://*.prayu.site/*` 의 `*` 는 `/` 를 못 넘어 `login-redirect?path=/group/<id>/join` 이 거부되고 Site URL(`app.prayu.site`) 루트로 대체됐다(`?handoff=<hex>` 는 `/` 가 없어 통과 → 원탭은 멀쩡). apex `prayu.site` 도 `*.` 패턴에 안 잡혔음. **조치**: 대시보드 `https://*.prayu.site/**` + `https://prayu.site/**` (사용자, 즉시 복구) · web 방어: `LogInDrawer` 가 `path` 를 `encodeURIComponent` (main/staging 만, prod 는 대시보드로 해결). 대장: [guides/supabase-migration-plan.md](guides/supabase-migration-plan.md) "Auth URL Configuration"
