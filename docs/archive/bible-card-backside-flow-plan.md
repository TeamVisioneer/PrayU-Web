...
> **상태 (2026-07): 구현 완료.** PR #447~#454로 반영됨. 일부 내용은 현실과 다름 — "독립 입력형 `/bible-card` 유지"는 #454에서 제거·어드민 격리로 변경, "DB 반영 추후 migration"은 baseline 마이그레이션으로 상환 완료.
> 후속 마무리 작업(재생성/생성 한도·프로필 CTA·잔재 정리)은 [bible-card-finishing-plan.md](./bible-card-finishing-plan.md) 참조.
